...
func getFlags() {
	...
	flag.StringVar(&secretName, "secret-name", "terraform-mutator-certs", "Name of the secret used to mount certs")
	flag.StringVar(&namespace, "namespace", "tf-system", "Namespace the service is deployed into")
	flag.StringVar(&mutatingWebhookConfigurationName, "mutating-webhook-configuration-name", "terraform-mutator", "Name of webhook resource")
	flag.StringVar(&webhookURI, "webhook-uri", "/terraform", "Name of webhook uri")
	flag.StringVar(&serviceName, "service-name", "terraform-mutator", "Name of the service to back up mutating webhook configuration")
	flag.Parse()
}
		...
		mutatingWebhook := addmissionregistrationv1.MutatingWebhook{
			NamespaceSelector: &metav1.LabelSelector{
				MatchExpressions: []metav1.LabelSelectorRequirement{
					{
						Key:      mutatingWebhookConfigurationName + "-exempt",
			...
			Rules: []addmissionregistrationv1.RuleWithOperations{
				{
					Operations: []addmissionregistrationv1.OperationType{addmissionregistrationv1.Create},
...
func main() {
	...
	// Setup Dynamic rest client to access v1alpha2
	...
	<-mgr.isReadyCh
	webhook.StartWebhook(tlsCertFilename, tlsKeyFilename, webhookURI, serviceName)
}
